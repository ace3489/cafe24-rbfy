$(function () {
  ...
  $('.scrollTo03').click(function (e) {
    ...
    $('html, body').animate(
      {
        scrollTop: $('#pdlistTab').offset().top - headerHeight - tabHeight - 10
      },
      300
    );
  });
  ...
  $pdlistTab.click(function () {
    ...
    // 패널 on 클래스 처리
    // $(`#pdlistPanel .panel[data-cate-no=${no}]`).addClass("on").siblings().removeClass("on");
    ...
    //     const newUrl = `${window.location.pathname}?cate_no=${no}`;
  ...
  // if(cateNo){
  //     $(`#pdlistTab button[data-cate-no=${cateNo}]`).trigger("click");
  // }
  ...
  // function getOffset(){
  //     return window.innerWidth <= 1024 ? 170 : 292;
  // }
  function getOffset() {
